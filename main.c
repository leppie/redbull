...
static void skin(ColorF* rgb, float x, float y)
{
	...
	ColorF carne = { .R = 0.75f, .G = 0.69f, .B = 0.6f };
...
static void eye(ColorF* rgb, float x, float y, float b)
 {
   ...
   ColorF fue = {.R = 1.0f, .G = 1.0f, .B = 1.0f};
   ...
   ColorF den = {.R = 0.35f, .G = 0.75f, .B = 0.45f+e };
...
/*
  ...
  while (1)
  {
    ...
    LCD_SetDisplayWindow(IMG_HEIGHT, IMG_WIDTH, IMG_HEIGHT - 1, IMG_WIDTH - 1);
    LCD_WriteReg(3, 0x1018);
    LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */
